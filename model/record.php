	...
	public function &__get($field) {
		if(!array_key_exists($field, $this->data)) {
			...
			if($result->num_rows > 1) {
				throw new TooManyResultsFoundException("Unexpected number of rows returned ({$result->num_rows}), expected exactly 1. Table:{$this->table}, ID field: {$this->idfield}, ID: {$this->id}");
			} elseif($result->num_rows < 1) {
				throw new NoResultsFoundException("Zero rows returned, expected exactly 1. Table:{$this->table}, ID field: {$this->idfield}, ID: {$this->id}");
			}
...
class TooManyResultsFoundException extends Exception {
	public function __construct($message) {
		parent::__construct($message);
	}
}
